...
### Password Handling
- Passwords are hashed with PHP's `password_hash()` (PASSWORD_DEFAULT) before storage — do NOT modify or uppercase passwords. Passwords are case-sensitive and must be preserved exactly as entered when verifying with `password_verify()`.
...
- ✅ Current status: `password_hash()` is used; migration from older SHA-256 hashes (if any) should be handled carefully and with a migration strategy (re-hash on next login or force reset).
...
## Agent-specific guidelines (for automated/code-assistant edits)

- Follow rules in `.github/instructions/AGENT.instructions.md` for commit format and push policy (always push to `dev`).
- **Do not commit `vendor/`**: this repository now ignores `vendor/` (see `.gitignore`); run `composer install` on the server or in CI instead of adding vendor files.
- **Migrations**: add SQL files in `database/migrations/` and document how to run them (manual `mysql < file.sql` or via a deploy script). Always include a short migration description in the commit message.
- **Encoding fixes**: use `UTF8::fix_utf8()` before `to_uppercase()` or `mb_strtoupper(..., 'UTF-8')`. When outputting to HTML, use `htmlspecialchars(..., ENT_QUOTES, 'UTF-8')`.
- **Uppercase policy**: use `app/helpers/uppercase_helper.php` and `get_uppercase_fields($table)` to determine which fields must be saved in uppercase. **Do not add `senha` (password) to uppercase fields.**
- **Password handling**: never modify or normalize password inputs. Use `password_hash()` and `password_verify()` patterns found in `app/controllers/create/UsuarioCreateController.php` and `app/controllers/update/UsuarioUpdateController.php`.
- **Quick checks before commit**: run `php -l` on changed PHP files; if you modify JS, run basic lint (if available). Add short `Testes:` section in commit message describing checks you ran.
- **Deployment preference**: prefer `git pull` and `composer install --no-dev --prefer-dist --no-interaction` on the server instead of FTP uploads for vendor management.

If anything here is unclear, open an issue or a PR referencing `.github/AGENT.instructions.md` for policy changes.
