...
- **Mapeamento correto de IDs**: telas agora usam `comum_id` (coluna `comum_id` em `produtos`) para ler/atualizar produtos, evitando confuso com legados `planilha_id`.
...
3. Visualize a planilha (`app/views/planilhas/planilha_visualizar.php`) e interaja com produtos sem recarregar a página.
...
O repositrio inclui um workflow do GitHub Actions que executa uma verificao de sintaxe PHP (php -l) em todos os arquivos PHP sob push e pull-requests. Para rodar localmente voc pode executar uma checagem rpida com:
...
Adicionar checks adicionais (PHPStan/PHPCS)  recomendado como prximo passo.
