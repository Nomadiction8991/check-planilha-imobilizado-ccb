...
$filtro_status = $_GET['filtro_status'] ?? ($_GET['filtro_STATUS'] ?? '');
...
// Buscar tipos de bens disponíveis para o select (SEM REPETIÁ‡Á•ES)
...
// Buscar códigos bem disponíveis para o select (SEM REPETIÁ‡Á•ES)
...
// Adicionar condiçÁµes de pesquisa
$condicoes = [];
...
// Adicionar condiçÁµes Á  query
if (!empty($condicoes)) {
    $sql .= " AND " . implode(" AND ", $condicoes);
    $sql_count .= " AND " . implode(" AND ", $condicoes);
}

// Ordenação por ID do maior para o menor (novos primeiro)
$sql .= " ORDER BY p.id_produto DESC LIMIT :limit OFFSET :offset";
...
function gerarParametrosFiltro($incluirPagina = false) {
    ...
    $filtroStatusParam = $_GET['filtro_status'] ?? ($_GET['filtro_STATUS'] ?? '');
    if (!empty($filtroStatusParam)) {
        $params['filtro_status'] = $filtroStatusParam;
    }
